...
context "Substitute" do
  context "One Record Methods" do
    ...
    events.zip(other_events).each do |(event, other_event)|
      ...
      one_record_method = :"one_#{event_type_method_cased}_record"

      context "#{one_record_method}" do
        ...
        context "One Record Matches" do
          record = substitute.public_send(one_record_method) do |*values|
        ...
        context "More Than One Record Matches" do
          test "Is an error" do
            assert_raises(Telemetry::Sink::Capture::MatchError) do
              substitute.public_send(one_record_method)
            end
          end
        end

        context "No Records Match" do
          any_record = substitute.public_send(one_record_method) { false }
